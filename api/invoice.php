...
if ($action === 'generate') {
    // Only superadmin can generate on demand
    $stmt = $conn->prepare("SELECT role FROM users WHERE id = ?");
    $stmt->execute([$_SESSION['user_id']]);
    $role = $stmt->fetchColumn();
    if ($role === 'superadmin' && isset($_POST['account_id'])) {
        ...
        $pdf->Ln(5);
        $pdf->Cell(0,10,'Compte: '.$accountId,0,1);
        ...
        header('Location: ../Batitrax/dashboard.php?view_account='.$accountId);
        exit;
    }
}
